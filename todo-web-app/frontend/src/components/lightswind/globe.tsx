...
const Globe: React.FC<GlobeProps> = ({
	...
	useEffect(() => {
		...
		const resolvedBaseColor: [number, number, number] =
			typeof baseColor === "string" ? hexToRgbNormalized(baseColor) : baseColor || [0.4, 0.6509, 1]; // Default if not provided or invalid hex

		const resolvedMarkerColor: [number, number, number] =
			typeof markerColor === "string" ? hexToRgbNormalized(markerColor) : markerColor || [1, 0, 0]; // Default if not provided or invalid hex
