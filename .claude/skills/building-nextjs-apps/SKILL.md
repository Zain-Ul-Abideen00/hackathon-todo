...
# Install Lightswind UI Package
pnpm add lightswind
# Set up Lightswind UI components in project.
npx lightswind@latest init
# Optional: Add components to project (animated, specialized, 3d).
npx lightswind@latest add --category animated
npx lightswind@latest add --category specialized
npx lightswind@latest add --category 3d
```
...
export async function createTask(prevState: any, formData: FormData) {
  ...
  await db.insert(tasks).values({ title: validated.data.title })
  revalidatePath("/tasks")
  return { success: true }
}

// Usage in component (React 19)
"use client"
...
async function TaskList() {
  // NOTE: fetch is NOT cached by default in Next.js 16
  const tasks = await fetch("https://api.example.com/tasks", {
    cache: "force-cache", // Opt-in to caching
    next: { tags: ['tasks'] } // Add tags for revalidation
  }).then(r => r.json())
...
- **styling-with-lightswind** - UI components for Next.js apps
...
- [references/nextjs-16-patterns.md](references/nextjs-16-patterns.md) - General Next.js 16 patterns
- [references/nextjs-16-server-actions.md](references/nextjs-16-server-actions.md) - Forms & Server Actions
- [references/nextjs-16-data-fetching.md](references/nextjs-16-data-fetching.md) - Caching & Fetching
