...
```json
{
  "mcpServers": {
    ...
    "fastapi-jwt": {
      "type": "reference",
      "path": "references/fastapi-jwt-integration.md",
      "description": "FastAPI Shared Secret JWT Verification"
    }
  }
}
...
```bash
pnpm add better-auth @better-auth/oidc-provider drizzle-orm
```
...
## Phase 2: FastAPI Integration Patterns

We support two distinct integration patterns for Python backends (see [references/fastapi-jwt-integration.md](references/fastapi-jwt-integration.md)):

### Pattern A: Shared Secret (Recommended for Hackathon)
- **Mechanism**: `HS256` signing using `BETTER_AUTH_SECRET`.
- **Pros**: Zero network overhead, simple env var config, fast.
- **Implementation**: Requires manual `jose` signing in frontend config.

### Pattern B: JWKS (Production)
- **Mechanism**: `RS256` signing with public key fetch.
- **Pros**: Key rotation, standard OIDC compliance.
- **Implementation**: Backend fetches `/.well-known/jwks.json`.
